...
ShopSphere is a full-featured, production-ready **MERN (MongoDB, Express, React, Node.js)** multi-vendor e-commerce platform. It provides role-based workspaces for customers, independent sellers, platform administrators, customer support representatives, and delivery partners.
...
|       |-- middleware/      # JWT authentication, role verification, upload, error handler
...
```ini
# Public frontend URL prefix - proxied to backend in development
VITE_API_URL=/api
...
The frontend will run on `http://localhost:5173` with automated proxying of `/api` requests to the backend.
...
- Product cataloging workflow powered by Google Gemini AI to analyze raw descriptions and predict categories, tags, specifications, and suggested pricing.
