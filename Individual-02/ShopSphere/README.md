...
- **Render:** in the backend service's **Environment** settings, configure `MONGODB_URI`, `JWT_SECRET`, `ADMIN_PASSWORD`, `SUPPORT_PASSWORD`, `DELIVERY_PASSWORD`, and `FRONTEND_URL=https://shopsphere-chi-virid.vercel.app,https://shopsphere-2hjy3wq2k-adamahansinis-projects.vercel.app`. Staff passwords must each be at least 12 characters and distinct. `JWT_SECRET` must contain at least 32 characters; generate a strong value locally with `node -e "console.log(require('node:crypto').randomBytes(48).toString('base64url'))"`, then paste it directly into Render without sharing or committing it. Set `GEMINI_API_KEY` to enable AI product generation. Email credentials (`EMAIL_HOST`, `EMAIL_PORT`, `EMAIL_USER`, `EMAIL_PASSWORD`, and optionally `EMAIL_FROM`) are only needed to send mail. Save the settings and redeploy the backend.
- Permanent staff accounts are created or synchronized from the three staff password variables at startup. Customer and seller accounts are never seeded; they authenticate against MongoDB records created through the normal registration flow. Missing AI credentials disable AI endpoints with a service-unavailable response; they do not prevent backend startup.
