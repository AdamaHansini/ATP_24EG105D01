...
- **Render:** set `FRONTEND_URL=https://shopsphere-chi-virid.vercel.app` and redeploy the backend. For local development plus production, the CORS setting allows `http://localhost:5173` and the configured `FRONTEND_URL`.
- Keep MongoDB, JWT, Gemini, staff-account, and email credentials only in Render/backend environment variables. Do not prefix them with `VITE_`.
